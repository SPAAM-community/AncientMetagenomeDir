...
plot_cumulative_timeline <- function(x, grouping){
  ...
  grouping_order <- timeline %>% 
    filter(publication_year == max(publication_year)) %>% 
    arrange(cumulative_sum) %>% 
    pull(group)
  
  ggplot(timeline %>% mutate(group = factor(group, levels = grouping_order)), aes(publication_year, cumulative_sum, fill = group)) +
    geom_bar(stat = "identity") +
    scale_fill_brewer(palette ="Set1") +
    labs(fill = deparse(substitute(grouping))) +
    theme_minimal()
}
