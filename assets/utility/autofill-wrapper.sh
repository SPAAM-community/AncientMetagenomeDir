...
usage() {
    echo "Usage: $0 [-t <ancientmetagenome-hostassociated|ancientmetagenome-environmental|ancientsingle-hostassociated>] [-p <project_name>]" 1>&2
    exit 1
}

valid_tables=("ancientmetagenome-hostassociated" "ancientmetagenome-environmental" "ancientsingle-hostassociated")
verbose=false
...
if [ $verbose ]; then
    echo RUNNING: AMDirT autofill -n "$table_name" "$hits" -l TEMPLATE_"$project_name"_"$table_name"_libraries.tsv
    echo ""
fi
