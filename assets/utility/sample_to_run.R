...
ena_exp_fields <- c(
  experiment_id = "IDENTIFIERS.PRIMARY_ID", submitter_id = "IDENTIFIERS.SUBMITTER_ID",
  experiment_title = "TITLE", study_primary_accession = "STUDY_REF.IDENTIFIERS.PRIMARY_ID",
  study_secondary_accession = "STUDY_REF.IDENTIFIERS.SECONDARY_ID",
  library_design_description = "DESIGN.DESIGN_DESCRIPTION", primary_sample_accession = "DESIGN.SAMPLE_DESCRIPTOR.IDENTIFIERS.PRIMARY_ID",
  secondary_sample_id = "DESIGN.SAMPLE_DESCRIPTOR.IDENTIFIERS.EXTERNAL_ID",
  library_name = "DESIGN.LIBRARY_DESCRIPTOR.LIBRARY_NAME", library_strategy = "DESIGN.LIBRARY_DESCRIPTOR.LIBRARY_STRATEGY",
...
call_ena_home <- function(url, v = FALSE) {
  if (v == T) { print(paste0("[DEBUG] querying URL:", url, collapse = ": ")) }
  url %>% GET(url = .) %>% content(encoding = "UTF-8") #%>% read_tsv
}
...
join_table_run_experiment <- function(run_tab, exp_tab) {
  if (v == T) { print("[DEBUG] Joining by 'known' columns") }
...
ena_column_cleanup <- function(x, fields, v = FALSE) {
  if (v == T) { print("[DEBUG] Selecting preferred fields") }
...
results_experiment <- get_experiment_metadata(results_run$experiment_accession)
results_final <- join_table_run_experiment(results_run, results_experiment) %>% ena_column_cleanup(cols_of_interest)

### MULTI
hostass_meta <- read_tsv("ancientmetagenome-hostassociated/ancientmetagenome-hostassociated.tsv") %>% filter(archive == "ENA" | archive == "SRA")
hostass_samacc <- hostass_meta %>% group_split(project_name) %>% map(., ~pull(.x, archive_accession))

hostass_final_res <- map(hostass_samacc, ~get_run_from_sample_metadata(.x, v = T)) %>% bind_rows()
